 ...
 * V5.3.11 (2026/07/08): [通訊升級與安全機制] 接收 Config 電壓/電流上下限，實作全域安全斷電機制。
 * V5.3.12 (2026/07/09): [系統穩定性定版] 全面統整實體按鈕與 WEB 虛擬按鈕的旗標交握。
 * V5.3.13 (2026/07/13): [計時與同步優化] 精確控制發送時序，啟用 UART0 作為 Debug 輸出。
 * V5.3.14 (2026/07/14): [除錯與穩定性優化] 加入循環時間監控與更精細的 Debug 訊息。
 * V5.3.15 (2026/07/15): [熔斷警示完美修復] 補發最後一筆異常 PD 封包，確保 WEB 警示視窗 100% 彈出。
 * V5.3.16 (2026/07/15): [除錯開關與電源狀態修正] 新增全域變數 g_u8DebugEnable 控制 Debug 輸出。
 * V5.3.17 (2026/07/16): [單機模式優化與協定除錯]
 * V5.3.18 (2026/07/17): [架構重構-電源情境隔離] 移除全域 g_u8StandaloneMode / 全域保護門檻，
 *   改用每個介面各自持有的 PowerCtx（電源情境）結構，杜絕「同類型就共用全域」造成的洩漏：
 *     1. Wiegand / TK2 / RS232 / Power Monitor 四個介面各自擁有獨立的電源狀態與保護門檻，
 *        不再共用 g_power_state / g_cfg_* / g_web_power_toggle_req 的隱性耦合。
 *     2. CF(設定門檻) 與 PW(WEB電源請求) 兩指令只在「目前註冊為 Networked 情境」時才會被採納，
...
void JIG_8CP_Command_Handler(const char* cmd_code, const char* data) {
    if (strcmp(cmd_code, "SC") == 0) {
        ...
        if (sscanf(data, "%d,%d,%d,%d,%d,%d", &y, &m, &d, &h, &min, &s) == 6) {
            ...
            rtc_time.year = y; rtc_time.month = m; rtc_time.date = d;
            rtc_time.hours = h; rtc_time.minutes = min; rtc_time.seconds = s;
        ...
        if (sscanf(data, "%d,%d,%d,%d,%d", &idx, &h, &m, &s, &en) == 5) {
            if (idx >= 0 && idx < 6) {
                g_alarms[idx].hours = h; g_alarms[idx].minutes = m;
                g_alarms[idx].seconds = s; g_alarms[idx].enabled = en;
            }
        }
    }
...
void Time_Set_Menu_Loop(void) {
    ...
    // =======================================================
    // [修正] 進入 Time Set 選單前，等待所有按鈕完全釋放
    // 避免從 MAIN 進入時，黃色按鈕殘留狀態造成誤退出
    // =======================================================
    while(((PF->PIN & BIT5) == 0) || 
          ((PF->PIN & BIT3) == 0) || 
          ((PF->PIN & BIT4) == 0) || 
    ...
    // 只初始化非返回鍵的狀態
    last_red   = (PA->PIN & BIT8) ? 1 : 0;
    last_blue  = (PF->PIN & BIT3) ? 1 : 0;
    last_green = (PF->PIN & BIT4) ? 1 : 0;

    while(1) {
        Global_Background_Tasks();

        if (g_force_alarm_menu) { 
            idx = 2; 
            Alarm_Menu_Loop(); 
            continue; 
        } 
        ...
        // =======================================================
        // [關鍵修正] 黃色按鈕：返回 MAIN
        // 改成和 MAIN 一樣：按下後，等待放開，才算返回
        // 這樣不會把 MAIN 進入時的放開動作誤認為返回
        // =======================================================
        if ((PF->PIN & BIT5) == 0) {
            Delay_ms(50);
...
//            char buf1[33], buf2[33], buf3[33], buf4[33];
						char buf1[33]= {0};char buf2[33]= {0};char buf3[33]= {0};char buf4[33]= {0};						
            ...
            if (g_u8WifiConnected) {
                ...
                snprintf(buf4, 33, "W:WIFI IP B:Rst R:Pwr Y:Exit");
						...
						Safe_Print_OLED_Smooth(0,  0, 63, 0x0F, "%s", buf1);   // 根本修復
						Safe_Print_OLED_Smooth(16, 0, 63, 0x0F, "%s", buf2);
						Safe_Print_OLED_Smooth(32, 0, 63, 0x0F, "%s", buf3);
						Safe_Print_OLED_Smooth(48, 0, 63, 0x04, "%s", buf4);
//						Safe_Print_OLED_Smooth(48, 0, 63, 0x0F, "1234567890ABCDEFGHIJKLMNOPQRSTUV");
            UI_Update(); 
            last_ui_update_ms = g_u32SystemMs; 
        }
...
void UART_Monitor_Test(uint32_t u32BaudRate) {
    PowerCtx ctx;
    PowerCtx_Init(&ctx, PWR_SCOPE_ISOLATED); // 從不註冊 g_pActiveNetworkedCtx -> WEB CF/PW 對此介面永遠無效
    ...
    // --- [V5.3.18] 修正計時 bug：一律使用硬體絕對時鐘 g_u32SystemMs，不再使用相對計數 loop_tick ---
    ...
    while(1) {
        ...
        if (ctx.power_state) {
            ...
            while(g_u2_rx_head != g_u2_rx_tail && rx_len < 127) {
                ...
                else rx_buf[rx_len++] = '.'; // 前置碼 0x02 將在此被正確轉化為 '.' 並安全累積
                
                last_rx_time = g_u32SystemMs;
                packet_started = 1;
            }
            
            // 包尾斷訊判斷：當已經開始收資料，且距離最後一次收字元超過 50ms，視為一包完整的卡號
...
#include <stdint.h>
#include <stdbool.h>
// [V5.3.18 架構重構-電源情境隔離] ... (保留原有的 PowerCtx 定義與函式 ...)

// [V5.3.19 新增] Wiegand/TK2 異常偵測常數
#define WIEGAND_TIMEOUT_MS          500  // 接收一組完整 Wiegand 資料的最大允許時間 (毫秒)
#define WIEGAND_MAX_BITS            64   // Wiegand 資料最大位元數上限
#define TK2_TIMEOUT_MS              1000 // 接收一組完整 TK2 資料的最大允許時間 (毫秒)
#define TK2_MAX_BITS                256  // TK2 資料最大位元數上限 (根據實際協議調整)
...
void Wiegand_Monitor_Test(void) {
    ...
    // [防線1] 設定 Quasi 模式後，強制輸出 1 開啟內部弱上拉，抵抗雜訊！
    ...
    // [V5.3.19 新增] Wiegand 異常偵測變數
    uint32_t wg_start_time = 0; // 記錄本次接收開始的時間
    uint32_t wg_last_activity_time = 0; // 記錄最後一次接收活動的時間
    bool wg_receiving_active = false; // 標記是否正在接收中
    uint8_t wg_bit_count = 0; // 本次接收的位元計數 (用於異常檢查，可能與 g_u8WiegandNum 有差異)

    // [防線2] 進場大掃除：徹底清空從 TK2 殘留過來的幽靈資料
    QUEUE_CLEAR(au64WG1);
    g_u8WiegandNum = 0;
    // [V5.3.19 新增] 初始化異常偵測變數
    ...
    while(1) {
        ...
        if (Check_Power_Toggle(&ctx)) {
            if (ctx.power_state) {
                ...
                QUEUE_CLEAR(au64WG1);
                g_u8WiegandNum = 0;
                // [V5.3.19 新增] 重置 Wiegand 狀態機變數
                wg_start_time = g_u32SystemMs;
                wg_last_activity_time = g_u32SystemMs;
                wg_receiving_active = false;
                wg_bit_count = 0;
                Delay_ms(50); // 等待繼電器接點與接地電位穩定，避開切換瞬間的雜訊窗口
                ...
                PC->DOUT &= ~BIT7;
                JigBeep(500);
                 // [V5.3.19 新增] 當電源關閉時，也重置 Wiegand 狀態機變數
                 wg_receiving_active = false;
                 wg_bit_count = 0;
            }
            Reset_Current_Filter();
        ...
        if (Check_Reset_Button()) {
            loop_tick = 1000;
            // [V5.3.19 新增] 按下重置按鈕時，也重置 Wiegand 狀態機變數
            wg_receiving_active = false;
            wg_bit_count = 0;
        }
        vCheckingTimeOut();

        // [V5.3.19 新增] 檢查 Wiegand 接收是否異常
        ...
        if (ctx.power_state && wg_receiving_active) {
            ...
            if (current_time - wg_start_time > WIEGAND_TIMEOUT_MS) {
                ...
                wg_receiving_active = false;
                wg_bit_count = 0;
                wg_start_time = current_time; // 重置開始時間
                wg_last_activity_time = current_time; // 重置活動時間
                // 可選：發出警告音或在 UI 顯示錯誤
                // JigForceBeep(200); // 短促警告音
            }
            // 2. 檢查位元數
            ...
            // 3. 檢查接收停滯 (如果長時間沒有新活動，也視為超時)
            else if (current_time - wg_last_activity_time > (WIEGAND_TIMEOUT_MS / 2)) { // 設定一半時間為停滯閥值
        ...
        // [V5.3.19 新增] 檢查是否開始接收 (g_u8WiegandNum > 0 且之前未在接收中)
        if (ctx.power_state && g_u8WiegandNum > 0 && !wg_receiving_active) {
             ...
             wg_bit_count = g_u8WiegandNum; // 初始化位元計數
        }
        // [V5.3.19 新增] 更新位元計數和最後活動時間 (如果正在接收中)
        if (ctx.power_state && wg_receiving_active && g_u8WiegandNum > wg_bit_count) {
             wg_bit_count = g_u8WiegandNum;
             wg_last_activity_time = current_time; // 更新最後活動時間
        }
        ...
        if (loop_tick >= 1000) {
            if (rx_count > 0) {
                ...
                else snprintf(data_str, sizeof(data_str), "W%02d:%X", (int)last_wg_bits, wg_l);
            } else {
                strcpy(data_str, "WAITING...");
            }
...
void TK2_Monitor_Test(void) {
    ...
    // [防線1] 設定 Quasi 模式後，強制輸出 1 開啟內部弱上拉
    GPIO_SetMode(PA, BIT10, GPIO_MODE_QUASI); PA->DOUT |= BIT10;
    GPIO_SetMode(PB, BIT5,  GPIO_MODE_QUASI); PB->DOUT |= BIT5;
    ...
    // [V5.3.19 新增] TK2 異常偵測變數
    uint32_t tk2_start_time = 0; // 記錄本次接收開始的時間
    uint32_t tk2_last_activity_time = 0; // 記錄最後一次接收活動的時間
    bool tk2_receiving_active = false; // 標記是否正在接收中
    uint8_t tk2_expected_length = 0; // 預期的資料長度 (可以根據起始符或協議規則估算)
    ...
    while(1) {
        ...
        if (Check_Power_Toggle(&ctx)) {
            if (ctx.power_state) {
                ...
                PC->DOUT &= ~BIT7;
                JigBeep(500);
                // [V5.3.19 新增] 當電源關閉時，也重置 TK2 狀態機變數
                tk2_receiving_active = false;
                tk2_expected_length = 0;
            }
            Reset_Current_Filter();
        ...
        if (Check_Reset_Button()) {
            loop_tick = 1000;
            // [V5.3.19 新增] 按下重置按鈕時，也重置 TK2 狀態機變數
            tk2_receiving_active = false;
            tk2_expected_length = 0;
        }
        vCheckingTimeOut();

        // [V5.3.19 新增] 檢查 TK2 接收是否異常
        uint32_t current_time = g_u32SystemMs;
        if (ctx.power_state && tk2_receiving_active) {
            ...
            if (current_time - tk2_start_time > TK2_TIMEOUT_MS) {
                ...
                tk2_receiving_active = false;
                tk2_expected_length = 0;
                tk2_start_time = current_time; // 重置開始時間
                tk2_last_activity_time = current_time; // 重置活動時間
                // 可選：發出警告音或在 UI 顯示錯誤
                // JigForceBeep(200); // 短促警告音
            }
            // 2. 檢查位元數 (這裡用 TK2Cnt 來近似位元數，根據協議調整邏輯)
            else if ((int)TK2Cnt > TK2_MAX_BITS) { // 或者根據 tk2_expected_length 判斷
            ...
            // 3. 檢查接收停滯 (如果長時間沒有新活動，也視為超時)
            else if (current_time - tk2_last_activity_time > (TK2_TIMEOUT_MS / 2)) { // 設定一半時間為停滯閥值
        ...
        if (TK2Cnt > 0) {
            if (TK2Cnt != last_tk2_cnt) {
                ...
                tk2_idle_tick++;
                if (tk2_idle_tick > 50) {
                    // [防線3] 關閉中斷！防止 JigBeep 引發二次刷卡，根除 02/ 現象
                    NVIC_DisableIRQ(GPIO_PAPBPGPH_IRQn);
        ...
        // [V5.3.19 新增] 檢查是否開始接收 (TK2Cnt > 0 且之前未在接收中)
        if (ctx.power_state && TK2Cnt > 0 && !tk2_receiving_active) {
             tk2_receiving_active = true;
             tk2_start_time = current_time;
             tk2_last_activity_time = current_time;
             // 這裡可以嘗試估算 tk2_expected_length，例如根據協議規則或第一次收到的幾個位元
             // tk2_expected_length = ... ; // 根據實際協議實現
        }
